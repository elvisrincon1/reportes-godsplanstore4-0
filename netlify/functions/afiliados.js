...
exports.handler = async function(event, context) {
  ...
  try {
    if (method === 'GET') {
      const rows = await runQuery('SELECT * FROM afiliados');
      db.close();
      return {
        statusCode: 200,
      ...
      const result = await runRun('INSERT INTO afiliados (nombre) VALUES (?)', [data.nombre]);
      db.close();
      return {
        statusCode: 201,
      ...
      await runRun('UPDATE afiliados SET nombre = ? WHERE id = ?', [data.nombre, id]);
      db.close();
      return {
        statusCode: 200,
      ...
      await runRun('DELETE FROM afiliados WHERE id = ?', [id]);
      db.close();
      return {
        statusCode: 204,
        body: '',
